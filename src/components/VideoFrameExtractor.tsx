...
import { Play, Pause, SkipBack, SkipForward, Download, Grid3X3 } from 'lucide-react';
...
const VideoFrameExtractor: React.FC<VideoFrameExtractorProps> = ({ videoFile, onFrameAnalysis }) => {
  ...
  const extractFrame = (time: number): string => {
  ...
  const downloadFrame = (dataUrl: string, timestamp: number) => {
    ...
    link.href = dataUrl;
    link.download = `frame_${formatTime(timestamp)}.jpg`;
    link.click();
  };
              ...
              {extractedFrames.map((frame, index) => (
                <div key={index} className="relative group">
                  <img
                    ...
                    className="w-full aspect-video object-cover rounded-lg shadow-soft hover:shadow-medium transition-shadow"
                  />
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors rounded-lg flex items-center justify-center opacity-0 group-hover:opacity-100">
                    <Button
                      onClick={() => downloadFrame(frame.dataUrl, frame.timestamp)}
                      size="sm"
                      variant="secondary"
